    ...
    parse(raw) {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (latencyIdx !== -1 && cols[latencyIdx]) {
          ...
          if (!isNaN(lat) && lat > 0) { stats.latencySum += lat; stats.latencyCount++; }
        }
  ...
  proxyscrape: {
    ...
    parse(raw) {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const port = (cols[portIdx] || "").trim();
        if (!ip || !port) continue;
        const protocol = (protoIdx !== -1 && cols[protoIdx]) ? cols[protoIdx].trim() : "http";
        const country = (countryIdx !== -1 && cols[countryIdx]) ? cols[countryIdx].trim() : "";
        items.push({ proxy: `${protocol}://${ip}:${port}`, country });
        stats.total++;
        if (country) stats.countries[country] = (stats.countries[country] || 0) + 1;
...
function getProvider(name) {
  ...
  if (!PROVIDERS[key]) {
    console.error(`  [provider] Unknown provider: "${key}". Available: ${Object.keys(PROVIDERS).join(", ")}`);
    process.exit(1);
  }
...
function testHttpConnect(proxyHost, proxyPort, targetHost, targetPort, timeout) {
  return new Promise((resolve) => {
    ...
    sock.on("data", (chunk) => {
      data += chunk.toString();
      if (data.includes("\r\n\r\n")) {
        if (!/HTTP\/1\.[01] 200/.test(data)) { sock.destroy(); return done(false); }
        const tlsSock = tls.connect(
          { host: targetHost, port: targetPort, socket: sock, servername: targetHost, timeout },
          () => { tlsSock.destroy(); done(true); },
        );
        tlsSock.on("error", () => { sock.destroy(); done(false); });
        tlsSock.on("timeout", () => { tlsSock.destroy(); done(false); });
      }
    });
    timer = setTimeout(() => { sock.destroy(); done(false); }, timeout);
...
function testSocks5(proxyHost, proxyPort, targetHost, targetPort, timeout) {
  return new Promise((resolve) => {
    const start = Date.now();
    let timer, settled = false;
    const done = (ok) => { if (settled) return; settled = true; clearTimeout(timer); resolve({ ok, ms: Date.now() - start }); };
    let step = 0;
    const sock = net.connect(proxyPort, proxyHost, () => { sock.write(Buffer.from([0x05, 0x01, 0x00])); });
    sock.on("data", (chunk) => {
      if (step === 0) {
        if (chunk.length >= 2 && chunk[0] === 0x05 && chunk[1] === 0x00) {
          ...
          buf[4] = hostBuf.length; hostBuf.copy(buf, 5);
          buf.writeUInt16BE(targetPort, 5 + hostBuf.length);
          sock.write(buf);
        } else { sock.destroy(); done(false); }
      } else if (step === 1) {
        if (chunk.length >= 2 && chunk[0] === 0x05 && chunk[1] === 0x00) {
          const tlsSock = tls.connect(
            { host: targetHost, port: targetPort, socket: sock, servername: targetHost, timeout },
...
function testSocks4(proxyHost, proxyPort, targetHost, targetPort, timeout) {
  return new Promise((resolve) => {
    ...
    const sock = net.connect(proxyPort, proxyHost, () => {
      ...
      buf[4] = 0x00; buf[5] = 0x00; buf[6] = 0x00; buf[7] = 0x01; buf[8] = 0x00;
      hostBuf.copy(buf, 9); buf[9 + hostBuf.length] = 0x00;
      sock.write(buf);
    });
    sock.on("data", (chunk) => {
      if (chunk.length >= 2 && chunk[0] === 0x00 && chunk[1] === 0x5a) { sock.destroy(); done(true); }
      else { sock.destroy(); done(false); }
    });
    timer = setTimeout(() => { sock.destroy(); done(false); }, timeout);
    sock.on("error", () => { sock.destroy(); done(false); });
...
async function testProxy(proxyUrl, timeout = DEFAULTS.timeout) {
  ...
  for (const t of TARGETS) {
    let r;
    if (protocol === "socks5") r = await testSocks5(host, port, t.host, t.port, timeout);
    else if (protocol === "socks4") r = await testSocks4(host, port, t.host, t.port, timeout);
...
function formatIsoToLocal(isoStr) {
  try { return formatIndonesianDate(new Date(isoStr)); } catch { return isoStr; }
}
...
function loadProxies(csvPath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (proxy && (proxy.startsWith("http") || proxy.startsWith("socks") || /^\d+\.\d+\.\d+\.\d+:\d+/.test(proxy)) && !seen.has(proxy)) {
      seen.add(proxy);
...
function dedup(csvPath) {
  if (!fs.existsSync(csvPath)) { console.error(`File not found: ${csvPath}`); return { before: 0, removed: 0, after: 0 }; }
...
async function checkOnce(items, opts = {}) {
  ...
  const results = await runBatch(items, concurrency, async (item, i) => {
    ...
    if (r.ok) { aliveCount++; timings.push({ proxy: proxyUrl, country: typeof item === "object" ? item.country : "", ms: r.ms }); }
    else deadCount++;
    if ((i + 1) % 10 === 0 || i === items.length - 1) {
      process.stdout.write(`  Checked ${i + 1}/${items.length} | \x1b[32mAlive: ${aliveCount}\x1b[0m | \x1b[31mDead: ${deadCount}\x1b[0m\r`);
    }
    return r.ok;
  });
...
async function deepClean(items, outputPath, opts = {}) {
  const { deadTarget = 0, concurrency = DEFAULTS.concurrency, timeout = DEFAULTS.timeout, roundDelay = DEFAULTS.roundDelay } = opts;
  ...
  while (true) {
    ...
    if (result.total === 0) { console.log("\n  No proxies left. Stopping."); saveProxies([], outputPath); break; }
    if (result.dead <= deadTarget) {
      ...
      if (result.timings.length > 0) {
        ...
        for (const t of result.timings) console.log(`    ${t.ms}ms  ${t.proxy}  [${t.country || "-"}]`);
      }
  ...
  console.log("------------------------------------------");
  for (const r of rounds) console.log(`  Round ${r.round}  |  Dead: ${r.dead}  |  Alive: ${r.alive}  |  Total: ${r.total}`);
  console.log("==========================================\n");
  return rounds;
}
...
async function fetchProxies(providerName, outputPath) {
  ...
  console.log(`  Fetched ${lines.length - 1} rows in ${elapsed}ms → ${outputPath}`);
...
function askMode() {
  ...
  return new Promise((resolve) => {
    ...
    rl.question("Enter 1, 2, or 3: ", (answer) => {
      rl.close();
      const a = answer.trim();
      resolve({ mode: a === "2" ? "deep" : a === "3" ? "dedup" : "normal", deadTarget: 0 });
    });
  });
}
...
async function run(opts = {}) {
  ...
  if (mode === "dedup") {
    ...
    if (!target) { console.error("No input/output file specified for dedup."); return; }
  ...
  if (shouldFetch) {
    ...
    if (path.basename(inputPath) === "raw.csv") {
      ...
      console.log(`  Parsed ${proxies.length} proxies from ${providerObj.name} (${inputPath})`);
    } else {
      proxies = loadProxies(inputPath);
    }
  }

  if (proxies.length === 0) { console.log("No proxies to check."); return; }
  ...
  if (mode === "normal") {
    ...
    await deepClean(proxies, outputPath, { deadTarget, concurrency, timeout, roundDelay });
  }
}
...
function loadBlacklist() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (cols.length >= 3) entries.push({ proxy: cols[0].trim(), timestamp: cols[1].trim(), banned_until: cols[2].trim(), reason: (cols[3] || "").trim() });
  }
  return entries;
}
...
function saveBlacklist(entries) {
  ...
  for (const e of entries) lines.push(`${e.proxy},${e.timestamp},${e.banned_until},${e.reason}`);
...
function isBlacklisted(proxy) {
  ...
  for (const e of entries) {
    ...
    if (until > now) { active.push(e); if (e.proxy === proxy) found = true; }
  }
...
function addToBlacklist(proxy, reason = "automated_queries", durationMinutes = 10) {
  ...
  if (existing !== -1) {
    ...
    entries.push({ proxy, timestamp: now.toISOString(), banned_until: until.toISOString(), reason });
  }
...
function cleanExpiredBlacklist() {
  ...
  if (active.length !== before) { saveBlacklist(active); console.log(`  [blacklist] Cleaned ${before - active.length} expired entries`); }
  return before - active.length;
}

module.exports = {
  ensureProtocol, parseProxyUrl, testProxy, testHttpConnect, testSocks5, testSocks4,
  runBatch, loadProxies, saveProxies, dedup, checkOnce, deepClean,
  fetchProxies, fetchFromUrl, getProvider, parseArgs, askMode, run,
  loadBlacklist, saveBlacklist, isBlacklisted, addToBlacklist, cleanExpiredBlacklist,
  formatIndonesianDate, formatIsoToLocal, printProviderStats,
  PROVIDERS, DEFAULT_PROVIDER, BLACKLIST_PATH, PROXIES_DIR, KEYS_DIR, TARGETS, DEFAULTS,
};
