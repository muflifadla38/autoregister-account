...
async function stepVerifyCaptcha(ctx) {
  ...
  console.log(
    `  Network intercepted config: sceneId="${capturedConfig?.sceneId}", prefix="${capturedConfig?.prefix}"`,
  );
  ...
  if (CONFIG.captchaMode === "capmonster") {
    ...
    if (!solved) {
      console.log(
        "  >>> CapMonster failed. Falling back to MANUAL solving (180s)...",
      );
      solved = await waitForQoderCaptchaSolved(
        oauthPage,
    ...
    // Manual mode
    console.log(
      "  >>> CAPTCHA: Solve the slider captcha MANUALLY in the browser.",
    );
    console.log(
      "  >>> Bot will auto-detect when solved. Waiting up to 180 seconds...",
    );
    solved = await waitForQoderCaptchaSolved(
      oauthPage,
      [
  ...
  if (!solved) {
    console.log("  [WARN] Timeout waiting for captcha. aborting...");
    process.exitCode = 1;
    return;
  }
